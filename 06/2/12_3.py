...
for m in tqdm(ms):
    mats = np.random.normal(scale=np.sqrt(m), size=(N_MAT, m, m))
    ...
    plt.ylim(0, 1.1)
    # plt.show()
    plt.savefig('./square_ecdf/{}.pdf'.format(m))
    plt.clf()
    ...
    plt.axis('equal')
    # plt.show()
    plt.savefig('./square_evs/{}.png'.format(m))
    plt.clf()
...
plt.plot(ms, conds, 'gx', label=r'$\kappa(A)$')
plt.plot(ms, s_mins, 'cx', label=r'$\sigma_{\mathrm{min}}$')

plt.xlim(xs[0], xs[-1])
plt.xlabel(r'$m$')
plt.legend()
plt.savefig('norm_spectral_cond.pdf')
plt.xlim(xs[0], 70)
plt.ylim(-5, 200)
plt.savefig('norm_spectral_cond_zoomed.pdf')
plt.clf()
